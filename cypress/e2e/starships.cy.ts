describe('Starships cards', () => {
  ...
  it('display starships game', () => {
    ...
    cy.get('[data-test=card]').should('have.text', '');
    cy.get('[data-test=card-spinner]').should('have.length', 0);
  });
  ...
  it('has 2 players (Player 1, Computer)', () => {
    ...
    cy.get('@player1').find('[data-test=score]').should('have.text', 'Score: 0');

    cy.get('@player2').contains('Computer');
    cy.get('@player2').find('[data-test=score]').should('have.text', 'Score: 0');
  });
  ...
  describe('Draw cards successfully', () => {
    beforeEach(() => {
      ...
      cy.intercept('**/api/starships/1', { delay: 1000, fixture: 'starships/starship-1.json' }).as('player1Request');
      cy.intercept('**/api/starships/2', { delay: 1000, fixture: 'starships/starship-2.json' }).as('player2Request');
    });

    it('display loaders while cards drawing', () => {
      cy.get('@playBtn').click();

      cy.get('@player1').find('[data-test=card-spinner]').should('have.length', 1);
      cy.get('@player2').find('[data-test=card-spinner]').should('have.length', 1);
    });
    ...
    it('display winner', () => {
      ...
      cy.get('@player1').find('[data-test=player-name]').should('not.contain', 'WIN');
    ...
    it('display score', () => {
      ...
      cy.get('@player1').find('[data-test=score]').should('have.text', 'Score: 0');
      cy.get('@player2').find('[data-test=score]').should('have.text', 'Score: 1');
    });
  ...
  describe('Draw cards failure', () => {
    ...
    it('draw cards failure (error from api/starships/*)', () => {
      cy.intercept('api/starships*', { fixture: 'starships/starship-collection-1.json' });
      cy.intercept('api/starships/*', { statusCode: 500 });
      cy.get('@playBtn').click();
      cy.get('[data-test=card]').should('have.text', '');
      cy.get('[data-test=card-spinner]').should('have.length', 0);
