describe('People cards', () => {
  ...
  describe('Draw cards successfully', () => {
    beforeEach(() => {
      ...
      cy.intercept('**/api/people/1', { delay: 1000, fixture: 'people/people-1.json' }).as('player1Request');
      cy.intercept('**/api/people/2', { delay: 1000, fixture: 'people/people-2.json' }).as('player2Request');
    });
    ...
    it('display winner', () => {
      ...
      cy.get('@player2').find('[data-test=player-name]').should('not.contain', 'WIN');
    });
    ...
    it('display score', () => {
      ...
      cy.get('@player1').find('[data-test=score]').should('have.text', 'Score: 1');
      cy.get('@player2').find('[data-test=score]').should('have.text', 'Score: 0');
    });
  ...
  describe('Draw cards failure', () => {
    ...
    it('draw cards failure (error from api/people/*)', () => {
      cy.intercept('api/people*', { fixture: 'people/people-collection-1.json' });
      cy.intercept('api/people/*', { statusCode: 500 });
      cy.get('@playBtn').click();
      cy.get('[data-test=card]').should('have.text', '');
      cy.get('[data-test=card-spinner]').should('have.length', 0);
